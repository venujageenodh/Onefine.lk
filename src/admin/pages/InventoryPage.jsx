...
export default function InventoryPage() {
    ...
    const [stockInForm, setStockInForm] = useState({ productId: '', qty: '', supplierId: '', reason: '' });
        ...
        try {
            await apiFetch('/inventory/stock-in', {
                method: 'POST', body: JSON.stringify({
                    ...stockInForm, qty: Number(stockInForm.qty)
                })
            }, token);
            setStockInForm({ productId: '', qty: '', supplierId: '', reason: '' });
            fetch();
        } catch (e) { alert(e.message); } finally { setSaving(false); }
    };
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 text-xs text-slate-500 uppercase tracking-wider">
                                <tr>
                                    {['Product', 'SKU', 'Stock', 'Reserved', 'Min Stock', 'Status', 'Action'].map(h => (
