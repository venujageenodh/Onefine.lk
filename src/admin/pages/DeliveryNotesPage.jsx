...
import { apiFetch, formatDateTime, StatusBadge, apiUrl } from '../utils';
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h2 className="font-bold text-[#1B2A4A] text-xl tracking-tight">Delivery Notes</h2>
            </div>

            {/* Filters */}
            <div className="flex flex-wrap gap-3 bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                <input placeholder="Search name / city / order #" value={filters.q}
            ...
            <div className="rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <span className="font-bold text-[#1B2A4A]">Processing for Delivery <span className="text-slate-400 font-normal">({total})</span></span>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-slate-50 text-xs text-slate-500 uppercase tracking-wider">
                            <tr>
                                {['Order #', 'Customer', 'City', 'Status', 'Date', 'Actions'].map(h => (
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {loading ? (
                                ...
                                <tr key={o._id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 font-medium text-slate-600">{o.customer?.city || o.customerCity || '—'}</td>
                            ...
                            {!loading && !orders.length && (
                                <tr><td colSpan={6} className="py-10 text-center text-slate-400">No matching orders found</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
